...
export const pt = {
  ...
  board: {
    ...
    roundsStatusLabel: "Estado das rondas",
    roundFinal: (name: string) => `${name} — final`,
    roundInProgress: (name: string) => `${name} — a decorrer`,
    roundUpcoming: (name: string) => `${name} — por jogar`,
    provisionalWin: "Na frente — não é definitivo até a ronda ser bloqueada",
